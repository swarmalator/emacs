...
(defcustom org-mind-map-edge-format-default "[style=dotted]"
  "Default format string for graph edges, e.g. \"[style=dotted]\"."
  :type 'string
  :group 'org-mind-map)
...
If there is a column summary value for the property that has recently be calculated it will be used."
  (let* ((node el)
    ...
    (if (and (equal prop :OMM-NODE-FMT) (not val)) "default" val)))
;; val))
...
in order to keep the tag colors consistent across calls."
  (org-element-map (org-element-parse-buffer 'object)
     ...
     (lambda (l)
       (if (org-mind-map-valid-link? l)
	   (let* ((f (org-mind-map-first-headline l))
		  (h (org-mind-map-destination-headline l))
		  (origin (org-mind-map-write-tags hm f))
		  (destination (org-mind-map-write-tags hm h)))
	     (if (string-greaterp
		  (org-element-property :raw-value h)
		  (org-element-property :raw-value f))
		 (list origin destination)
	       )
	     )))
     ))
...
;;	 (links (org-mind-map-get-links hm)))
